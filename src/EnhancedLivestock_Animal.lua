...
function Animal.new(age, health, monthsSinceLastBirth, gender, subTypeIndex, reproduction, isParent, isPregnant, isLactating, clusterSystem, id, motherId, fatherId, pos, name, dirt, fitness, riding, farmId, weight, genetics, impregnatedBy, variation, children, monitor, isCastrated, diseases, recentlyBoughtByAI, marks, insemination)
    ...
    if self.age >= 0 then
        ...
        self.birthday = {
            ["day"] = math.random(1, EnhancedLivestock.DAYS_PER_MONTH[birthMonth]),
            ["month"] = birthMonth,
            ["year"] = birthYear,
...
function Animal:addInfos(infos)
    ...
    if self:getSupportsReproduction() then
        ...
        if self.infoReproduction.disabled then
            ...
            if self.age < subType.reproductionMinAgeMonth then
                attributeText = g_i18n:getText("el_ui_tooYoung")
                valueText = g_i18n:formatNumMonth(subType.reproductionMinAgeMonth)
            elseif self.isParent and self.monthsSinceLastBirth <= 2 then
                attributeText = g_i18n:getText("el_ui_recoveringLastBirth")
                valueText = g_i18n:formatNumMonth(3 - self.monthsSinceLastBirth)
            elseif not EnhancedLivestock.hasMaleAnimalInPen(self.clusterSystem, subType.name, self) and self.reproduction == 0 then
                attributeText = g_i18n:getText("el_ui_noMaleAnimal")
                valueText = "0"
            elseif healthFactor < subType.reproductionMinHealth then
                attributeText = g_i18n:getText("el_ui_unhealthy")
    ...
    if hasMonitor then
        ...
        if self.infoWeight == nil then
            self.infoWeight = {
                text = g_i18n:getText("el_ui_weight"),
    ...
    if self.gender ~= nil and self.gender == "female" then
        ...
        if self.infoPregnant == nil then
            self.infoPregnant = {
                text = g_i18n:getText("el_ui_pregnant"),
        ...
        self.infoPregnant.valueText = self.isPregnant and g_i18n:getText("el_ui_yes") or g_i18n:getText("el_ui_no")
        ...
        if pregnancy ~= nil and pregnancy.pregnancies and #pregnancy.pregnancies > 0 then
            ...
            if self.infoPregnancyExpecting == nil then
                self.infoPregnancyExpecting = {
                    text = g_i18n:getText("el_ui_pregnancyExpecting"),
                    title = g_i18n:getText("el_ui_pregnancyExpecting"),
                    value = 1,
                    ratio = 1
                }
            end
            ...
            if self.infoPregnancyExpected == nil then
                self.infoPregnancyExpected = {
                    text = g_i18n:getText("el_ui_pregnancyExpected"),
                    title = g_i18n:getText("el_ui_pregnancyExpected"),
                    value = 1,
                    ratio = 1
                }
            end

            self.infoPregnancyExpecting.valueText = string.format("%s %s", #pregnancy.pregnancies, g_i18n:getText("el_ui_pregnancy" .. (#pregnancy.pregnancies == 1 and "Baby" or "Babies")))
            self.infoPregnancyExpected.valueText = string.format("%s/%s/%s", pregnancy.expected.day, pregnancy.expected.month, pregnancy.expected.year + EnhancedLivestock.START_YEAR.FULL)
        ...
        if self.isLactating ~= nil and hasMonitor and self.age > 12 and self.clusterSystem ~= nil and self.clusterSystem.owner.spec_husbandryMilk ~= nil then
            ...
            if self.infoLactation == nil then
                self.infoLactation = {
                    text = g_i18n:getText("el_ui_lactating"),
            ...
            self.infoLactation.valueText = self.isLactating and g_i18n:getText("el_ui_yes") or g_i18n:getText("el_ui_no")
...
function Animal:showInfo(box)
    ...
    local yesText = g_i18n:getText("el_ui_yes")
    local noText = g_i18n:getText("el_ui_no")
    ...
    if self.birthday ~= nil then
        ...
        local birthday = self.birthday
        box:addLine(g_i18n:getText("el_ui_birthday"), string.format("%d/%d/%d", birthday.day, birthday.month, EnhancedLivestock.START_YEAR.FULL + birthday.year))
    ...
    if self.gender ~= nil and self.gender == "female" and subType.supportsReproduction then
        ...
        if pregnancy ~= nil and pregnancy.pregnancies and #pregnancy.pregnancies > 0 then
            ...
            box:addLine(g_i18n:getText("el_ui_pregnancyExpecting"), string.format("%s %s", #pregnancy.pregnancies, g_i18n:getText("el_ui_pregnancy" .. (#pregnancy.pregnancies == 1 and "Baby" or "Babies"))))
        ...
        if self.age < subType.reproductionMinAgeMonth then
            text = g_i18n:getText("el_ui_tooYoungBracketed")
        elseif self.isParent and self.monthsSinceLastBirth <= 2 then
            text = g_i18n:getText("el_ui_recoveringLastBirthBracketed")
        elseif self.clusterSystem ~= nil and not EnhancedLivestock.hasMaleAnimalInPen(self.clusterSystem.owner.spec_husbandryAnimals, name, self) and not self.isPregnant then
            text = g_i18n:getText("el_ui_noMaleAnimalBracketed")
        elseif healthFactor < subType.reproductionMinHealth then
            text = g_i18n:getText("el_ui_unhealthyBracketed")
        end
    ...
    elseif self.gender ~= nil and self.gender == "male" and subType.reproductionMinAgeMonth ~= nil and self.age >= subType.reproductionMinAgeMonth then
        ...
        box:addLine(g_i18n:getText("el_ui_maleNumImpregnatable"), string.format("%s", self.numImpregnatableAnimals or 0))
    end
...
function Animal:showGeneticsInfo(box)
    ...
    box:addLine("el_ui_overall", "el_ui_genetics_" .. qualityText)
    ...
    box:addLine(g_i18n:getText("el_ui_metabolism"), "el_ui_genetics_" .. qualityText)
    ...
    box:addLine(g_i18n:getText("el_ui_health"), "el_ui_genetics_" .. qualityText)
    ...
    box:addLine(g_i18n:getText("el_ui_fertility"), "el_ui_genetics_" .. qualityText)
    ...
    box:addLine(g_i18n:getText("el_ui_meat"), "el_ui_genetics_" .. qualityText)
...
function Animal:addGeneticsInfo()
    ...
    text = {
        title = g_i18n:getText("el_ui_overall"),
        text = "el_ui_genetics_" .. qualityText
    }
    ...
    text = {
        title = g_i18n:getText("el_ui_metabolism"),
        text = "el_ui_genetics_" .. qualityText
    }
    ...
    text = {
        title = g_i18n:getText("el_ui_health"),
        text = "el_ui_genetics_" .. qualityText
    }
    ...
    text = {
        title = g_i18n:getText("el_ui_fertility"),
        text = "el_ui_genetics_" .. qualityText
    }
    ...
    text = {
        title = g_i18n:getText("el_ui_meat"),
        text = "el_ui_genetics_" .. qualityText
    }
...
function Animal:showMonitorInfo(box)
    ...
    box:addLine(g_i18n:getText("el_ui_monitorFee"), string.format(g_i18n:getText("el_ui_feePerMonth"), g_i18n:formatMoney(self.monitor.fee, 2, true, true)))
    ...
    box:addLine(g_i18n:getText("el_ui_targetWeight"), string.format("%.2f", self.targetWeight) .. "kg")
    box:addLine(g_i18n:getText("el_ui_valuePerKilo"), g_i18n:formatMoney(self:getSellPrice() / self.weight, 2, true, true))

    for fillType, amount in pairs(self.input) do

        box:addLine(g_i18n:getText("el_ui_input_" .. fillType), string.format(g_i18n:getText("el_ui_amountPerDay"), (amount * 24) / daysPerMonth))
    ...
    for fillType, amount in pairs(self.output) do
        ...
        box:addLine(g_i18n:getText("el_ui_output_" .. outputText), string.format(g_i18n:getText("el_ui_amountPerDay"), (amount * 24) / daysPerMonth))
    ...
    if insemination ~= nil and g_server ~= nil then
        ...
        if childNum > 0 and math.random() >= (2 - fertility) * 0.25 and math.random() <= insemination.success * (math.random(80, 120) / 100) then
            ...
            self:createPregnancy(childNum, month, year, {
                ["uniqueId"] = string.format("%s %s %s", EnhancedLivestock.AREA_CODES[insemination.country].code, insemination.farmId, insemination.uniqueId),
...
function Animal:getIdentifiers()
    ...
    return string.format("%s %s %s", EnhancedLivestock.AREA_CODES[self.birthday.country].code, self.farmId, self.uniqueId)
...
function Animal:deleteVisual()
    ...
    if self.visualAnimal ~= nil and type(self.visualAnimal.delete) == "function" then
        self.visualAnimal:delete()
    end
